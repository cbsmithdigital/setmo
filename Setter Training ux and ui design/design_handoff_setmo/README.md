...
1. Read `docs/SetMo_PRD.md` first — it's the **behavioral source of truth** (roles, flows, pricing, scope, tech stack, constraints).
2. Use `DATA_MODEL.md` to generate migrations/schema, and `API_SURFACE.md` to scaffold route handlers + background jobs.
3. Open `prototype/SetMo App.html` in a browser to see exactly what each screen looks like and how it behaves. Use the **workspace switcher** (bottom-left) to move between Setter / Office / Group / Platform views.
4. Build the backend against the schema + endpoints, then wire the real UI (see "About the design files").
