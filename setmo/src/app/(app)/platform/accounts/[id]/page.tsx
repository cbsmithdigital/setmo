...
const ROLE_LABEL: Record<string, string> = { SETTER: "Setter", OFFICE_ADMIN: "Office manager", GROUP_ADMIN: "Group admin", SUPPORT: "Support", PLATFORM_ADMIN: "Platform admin" };
    ...
    <>
      ...
      <div className="content">
        ...
        <div className="card card-pad rise" style={{ marginBottom: 18 }}>
          ...
          {a.locations.map((l, i) => (
            <div key={l.id} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr", gap: 12, alignItems: "center", padding: "11px 4px", borderTop: i ? "1px solid var(--line-soft)" : "none" }}>
              <div>
              ...
              <div style={{ fontSize: 13.5, color: l.balanceMin < 0 ? "var(--amber)" : "var(--text-1)" }}>{l.balanceMin.toLocaleString()} min left</div>
              <div className="muted" style={{ fontSize: 13 }}>{l.burnPerDay.toFixed(1)} min/day</div>
              <div className="muted" style={{ fontSize: 13 }}>{l.daysToEmpty != null ? `${l.daysToEmpty}d left` : "—"}</div>
            </div>
          ))}
        </div>
        ...
        <div className="grid g-2">
          ...
          <div className="card card-pad rise">
            ...
            {a.users.slice(0, 25).map((u, i) => (
              <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 4px", borderTop: i ? "1px solid var(--line-soft)" : "none" }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 600, fontSize: 13.5 }}>{u.name || u.email}</div>
                  <div className="muted" style={{ fontSize: 11.5 }}>{u.email} · {u.location}</div>
                </div>
                <span className="chip" style={{ padding: "2px 9px", fontSize: 11 }}>{ROLE_LABEL[u.role] ?? u.role}</span>
                {u.status !== "ACTIVE" && <span className="muted" style={{ fontSize: 11 }}>{u.status.toLowerCase()}</span>}
              </div>
            ))}
          </div>
