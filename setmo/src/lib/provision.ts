  ...
  try {
    if (opts.kind === "practice") {
      const office = opts.claimOfficeId
        ...
        : await prisma.office.create({ data: { name: opts.practiceName, isProspect: false } });
      ...
      const office = opts.claimOfficeId
        ...
        : await prisma.office.create({ data: { name: opts.practiceName, isProspect: false, organizationId: org.id } });
